...
for line in file1:
	...
	whole_school=list(map_object)
file1.close()
whole_school=[3,4,3,1,2]
...
for bound in range(0,10):
	school = [bound]
	school_temp = school.copy()	

	for day in range(0, time):
		print(f"\nTotal lantern fish {len(school)}")
		if day < 2:
			print(f"Total lanternfish: {len(school)} After {day} day {school}")
		else:
			print(f"Total lanternfish: {len(school)} After {day} days {school}")
		for index, fish in enumerate(school):
			school_temp[index] -= 1
			if school_temp[index] < 0:
				school_temp[index] = 6
				school_temp.append(8)
			school = school_temp.copy()
	print(lookup)	
	lookup.append(len(school))

print(lookup)

for fish in whole_school:
	total = total + lookup[fish]
